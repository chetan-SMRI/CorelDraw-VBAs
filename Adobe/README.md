...
- Labels every panel as `Part X / Total` so installers can see the complete sequence.
...
- Creates one final A4 landscape visual summary: centered filename, centered job size/overlap/total, and a proportional thumbnail of every actual cropped panel with a compact `Part X/Total | W: 89in` label. Up to five panels use one row; 6–10 panels use two balanced rows.
...
4. Select `SMRI_PhotoshopPanelMaker_RawCode.jsx`.
...
6. Photoshop opens one new document for each panel and one final A4 landscape summary. The source remains open and unchanged.
...
Quit Photoshop, then copy `SMRI_PhotoshopPanelMaker_RawCode.jsx` or the distributable `SMRI_PhotoShop_PanelMaker.jsx` into Photoshop's `Presets/Scripts` folder.
...
Restart Photoshop. Run it from **File > Scripts** using the installed script name.
...
- `SMRI_PhotoshopPanelMaker_RawCode.jsx` — readable development source.
...
1. Copy `SMRI_PhotoShop_PanelMaker.jsx` into Photoshop's `Presets/Scripts` directory listed above.
2. Restart Photoshop.
3. It will appear under **File > Scripts > SMRI_PhotoShop_PanelMaker**; Browse is no longer required.
...
3. Start recording.
4. Choose **File > Scripts > SMRI_PhotoShop_PanelMaker** and cancel at its first prompt after it starts.
5. Stop recording.
